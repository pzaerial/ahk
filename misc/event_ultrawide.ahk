...
DeployZoomedOut() {
	; Global definitions
	global numberOfHeroes, castlePresent

	; Event Troop 1 (x8)
	MouseClick, left, 730, 1250
	sleep 50
	ClickOnLine(1480, 80, 790, 790, 8)
	sleep 250
	
	; Event Troop 2 (x4)
	MouseClick, left, 930, 1250
	sleep 50
	ClickOnLine(1480, 80, 790, 790, 4)
	sleep 250
	
	; Normal Troop 1 (x10)
	MouseClick, left, 1130, 1250
	sleep 50
	...
	; Heroic/Special
	castleBaseLocation := 1310
	firstHeroLocation := (castlePresent = 1) ? castleBaseLocation + 200 : castleBaseLocation
	Loop, %numberOfHeroes%
	{
		heroOffset := (A_Index - 1) * 200
		MouseClick, left, firstHeroLocation + heroOffset, 1250
		sleep 50
		MouseClick, left, 1060 + (A_Index * 10), 375 + (A_Index * 3)
		sleep 150 + (A_Index * 50)
	}


	; Spells (x5), location based on number of heroes.
	spellBaseLocation := 1510
	firstSpellLocation := (castlePresent = 1) ? spellBaseLocation + 200 : spellBaseLocation
	MouseClick, left, spellBaseLocation, 1250
	sleep 50
